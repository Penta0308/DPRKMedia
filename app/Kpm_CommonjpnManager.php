...
/**
 *  Kpm_CommonjpnManager
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Kpm
 */
class Kpm_CommonjpnManager extends Ethna_AppManager
{
	public function getLastestDate($lvl = null, $authID = null)
	{
		$sql = <<< 'SQL'
			SELECT
				...
				DATE_FORMAT(MAX(JunsongDateTime), '%Y年 %m月 %d日 ') AS date,
				TIME_FORMAT(MAX(JunsongDateTime), '%H時 %i分') AS time,
	...
	public function getLatestNews($limit = 5)
	{
		...
		$sql = <<< 'SQL'
			...
			WHERE
				...
				AND LanguageID = 201
				AND importance > 0
				AND InputDateTime >= date_add(?, interval -60 day)
			ORDER BY
...
SQL;
		...
		foreach ($list as $idx => $w) {
			$sql = "SELECT BalHengYear, GwonHo FROM journalsarticle WHERE JournalID = ".$w['MediaID']." ORDER BY BalHengYear DESC, GwonHo DESC LIMIT 1";
			$db =& $this->backend->getDB();
			$db->query("SET NAMES utf8;");
			$result =& $db->query($sql);
			$val = $result->fetchRow();
			...
			switch($w['MediaID']) {
				case 2020:
					...
					break;
				default:
					if ($val['BalHengYear'] >= 2000) {
						$list[$idx]['issue'] = sprintf("(%d/%d)", $val['BalHengYear']-2000, $val['GwonHo']);
					}
			}
		}
...
SQL;
	...
	public function getArticleKPTodayAllList($cnt, $day)
	{
		$lastestDate = $this->getLastestDate();
	...
	public function getArticleAllListByMediaID($id, $offset, $limit)
	{
		...
		$sql = <<< 'SQL'
			...
			WHERE
				...
				AND LanguageID = 201
				AND importance > 0
				AND JunsongDateTime >= date_add(?, interval -90 day)
			ORDER BY
				JunsongDateTime DESC
			LIMIT ?
			OFFSET ?
...
SQL;
	...
	public function getCategoryInfo($id, $category)
	{
		if (strtolower($category) == "section") {
			$sql = "SELECT SectionNameJpn AS name, lim_date from sections WHERE SectionID = ?";
		} else {
			$sql = "SELECT LocalNameJpn AS name, lim_date from locals WHERE LocalID = ?";
		}
...
SQL;
		...
		if ($keyword != "" && count($keywordList) > 0) {
