...
/**
 *  listjournal view implementation.
 *
 *  @author     {$author}
 *  @access     public
 *  @package    Kpm
 */
class Kpm_View_Listjournal extends Kpm_ViewClass
{
    ...
    public function preforward()
    {
		...
		$issueYear        = $this->af->get('year');
		$issueNum         = $this->af->get('num');
		$offset           = $this->af->get('offset') == null ? 0 : (int)$this->af->get('offset');
		$mediaName        = $commManager->getMediaName($mediaID);
		$yearList         = $commManager->getJournalIssueYearList($mediaID);
		if ($issueYear != ""){
			$numList          = $commManager->getJournalIssueNumList($mediaID, $issueYear);
		}else{
			$numList          = array();
		}
		$articleList = $commManager->getJournalListSearch($mediaID, $issueYear, $issueNum, $offset, LINES_PER_PAGE);
		...
		$this->af->setApp('mediaName',        $mediaName);
		$this->af->setApp('yearList',         $yearList);
		$this->af->setApp('numList',          $numList);
	...
	function getPager($total, $offset, $limit)
	{
		...
		$this->af->setApp('link', '/listjournal');
